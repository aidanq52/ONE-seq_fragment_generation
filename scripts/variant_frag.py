...
def print_single_mismatch(input_file="intermediate_files/4a_deduplicated_file_with_sequences.txt",
    ...
    confirm = input("\nDo you confirm these are the on-target sites? (yes/no): ").strip().lower()
    ...
    if confirm == "yes":
        ...
        for idx, row in enumerate(filtered.itertuples(index=False)):
            ...
            # Replace ID (first column in your file)
            first_col_name = df.columns[0]
            new_row[first_col_name] = f"{chr(65 + idx)}-variant"  # A-variant, B-variant, etc.
        ...
        # Combine: variants at the top, then the original df
